...
from ..dist.dataloader import data_aug
from ..dist.dist_net import DIST

from .post_proc import post_process
from .pre_proc import process_fn
from core.utils import (
    fetch_inst_centroid_maskrcnn,
    find_files,
    fn_time,
    load_gt,
    load_img,
    loads_model,
    transfer_inst_format,
)

## hovernet
from ..hovernet.infer.tile_pannuke import InferManager
...
@fn_time
def dist_predict_oneimg(
    img_path, dataset_name, model_name, model_path_dict, model=None, load_model=True
):
    if load_model:
        model = loads_model_dynamic(model_name, dataset_name, model_path_dict)
...
def dist_predict_oneimg_for_plot(
    img_path, dataset_name, model_name, model_path_dict, model=None, load_model=True
):
    if load_model:
        model = loads_model_dynamic(model_name, dataset_name, model_path_dict)
...
def seg_predict_oneimg_for_plot(
    img_path,
    dataset_name,
    model_name,
    model_path_dict,
...
def hovernet_predict_dir(pred_dir, dataset_name, model_name, model_path_dict):
    # run_args
    # input_dir= "/root/autodl-tmp/archive/datasets/{}/patched/coco_format/images/test"\
    #   .format(dataset_name)
...
def predict_oneimg(
    file, dataset_name, model_name, model_path_dict, model=None, load_model=True
):
    if model_name in ["mask_rcnn", "cascade_rcnn"]:
        pred, true = mask_rcnn_predict_oneimg(
            file, dataset_name, model_name, model_path_dict, model, load_model
        )
        return pred, true
    elif model_name == "dist":
        pred, true = dist_predict_oneimg(
            file, dataset_name, model_name, model_path_dict, model, load_model
        )
        return pred, true
    elif model_name == "seg_unet":
        pred, true = seg_predict_oneimg(
            file, dataset_name, model_name, model_path_dict, model, load_model
        )
        return pred, true
    elif model_name == "mmseg_fcn":
        pred, true = mmseg_predict_oneimg(
            file, dataset_name, model_name, model_path_dict, model, load_model
        )
        return pred, true
    elif model_name == "hovernet":
        pred, true = hovernet_predict_dir(
            file, dataset_name, model_name, model_path_dict
        )
        return pred, true
    else:
        print("No matched models")
...
def predict_dir(
    ...
    # pred_dir = "/root/autodl-tmp/archive/datasets/{}/patched/coco_format/images/test"\
    # .format(dataset_name)
    if load_model:
        model = loads_model_dynamic(model_name, dataset_name, model_path_dict)
    torch.cuda.empty_cache()
    ...
    for file in files[:length]:
        try:
            pred, true = predict_oneimg(
                file, dataset_name, model_name, model_path_dict, model, load_model=False
            )
            preds.append(pred)
            trues.append(true)
