...
from src.core.infer_base import hovernet_predict_dir, predict_dir, predict_oneimg
from src.core.stats_utils import (
    eveluate_one_pic_class,
    eveluate_one_pic_inst,
...
def evalute_oneimg(img_path, dataset_name, model_name, model_path_dict):
    ...
    # pprint(metric0)
    # evaluate class
    metric1 = eveluate_one_pic_class(
        true["centroids"], pred["centroids"], true["labels"], pred["labels"]
    )
    # print(metric1)
    # evaluate inst
...
def evalute_oneimg_v2(pred, true, metric2=None):
    ...
    metric0 = metric.compute()
    metric0 = (
        pd.DataFrame(metric0, index=[0], columns=["map_50", "map_75"])
        .values[0]
        .tolist()
    )
...
def evalute_overall(
    ...
    if model_name == "hovernet":
        img_names, preds_result, trues_result = hovernet_predict_dir(
            pred_dir, dataset_name, model_name, model_path_dict
        )
    else:
        img_names, preds_result, trues_result = predict_dir(
            pred_dir, dataset_name, model_name, model_path_dict
        )
    ...
    metrics0 = (
        pd.DataFrame(metrics0, index=[0], columns=["map_50", "map_75"])
        .values[0]
        .tolist()
    )
    # pprint(metrics0)
    ...
    if is_detail:
        ...
        if ex_name is not None:
            detail_result.to_csv(
                "/root/autodl-tmp/archive/core/metrics/evaluate_gen/{}_{}_{}_detail.csv".format(
                    dataset_name, model_name, ex_name
                )
            )
        else:
            detail_result.to_csv(
                "/root/autodl-tmp/archive/core/metrics/evaluate_result/{}_{}_detail_p2.csv".format(
    ...
    columns = (
        ["map_50", "map_75"]
        + ["acc", "f1"]
        + [str(i) for i in type_uid_list]
        + ["dice", "aji", "aji_plus", "dq", "sq", "pq"]
    )
    overall_result = pd.DataFrame(
        np.concatenate([metrics0, metrics1, metrics2])[None, :], columns=columns
    )

    if ex_name is not None:
        overall_result.to_csv(
            "/root/autodl-tmp/archive/core/metrics/evaluate_gen/{}_{}_{}_overall.csv".format(
                dataset_name, model_name, ex_name
            )
        )
    else:
        overall_result.to_csv(
            "/root/autodl-tmp/archive/core/metrics/evaluate_result/{}_{}_overall_p2.csv".format(
                dataset_name, model_name
            )
        )
...
def evalute_overall_im(
    ...
    if model_name == "hovernet":
        img_names, preds_result, trues_result = hovernet_predict_dir(
            pred_dir, dataset_name, model_name, model_path_dict=None
        )
    else:
        img_names, preds_result, trues_result = predict_dir(
            pred_dir,
...
def batch_tasks_old():
    ...
    eval_tasks = tasks.query("is_enable == 1")[
        ["dataset_name", "model_name", "predict_fn"]
    ].to_dict("records")
    ...
    for task in eval_tasks:
        task.update(
            {
                "pred_dir": "/root/autodl-tmp/archive/datasets/{}/patched/coco_format/images/test".format(
                    task["dataset_name"]
                )
            }
        )
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        for idx, e in zip(
            range(len(eval_tasks)),
            executor.map(
                evalute_overall,
                [task["pred_dir"] for task in eval_tasks],
...
def evaluate_type(
    dataset_name, model_name, model_path_dict, type_uid_list=[0, 1, 2, 3, 4]
):
    """
    ex_name : 标记数据集名称
    """
