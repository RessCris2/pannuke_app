...
def visualize_instances_map(
    ...
    for inst_idx, inst_id in enumerate(inst_list):
        ...
        if type_map is not None:
            ...
            if type_id > len(type_colour)-1: # 如果出现别的i
                continue 
...
if __name__ == "__main__":
    import cv2
    input_image =  cv2.imread("/root/autodl-tmp/datasets/consep/images/train/000.jpg")
    la = np.load("/root/autodl-tmp/datasets/consep/labels_inst/train/000.npy")
    overlay = visualize_instances_map(input_image, la[:,:,0])