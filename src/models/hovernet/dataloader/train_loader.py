...
class FileLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inst_path = path.replace("images", 'inst').replace("jpg", 'npy')
        type_path = path.replace("images", 'seg_mask').replace("jpg", 'png')
        img = load_img(path)
        inst_mask = load_img(inst_path)[...,None]
        type_mask = load_img(type_path)[..., None]

        data = np.concatenate([img, inst_mask, type_mask], axis=-1)
