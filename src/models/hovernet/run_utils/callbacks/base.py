    ...
    def run(self, state, event):
        ...
        for key, step_value in step_output.items():
            ...
            if key in accumulated_output:
                #
                if (key=='true_np' ) and  step_value.ndim < 3:
                # if (key=='true_np' or key=='true_hv') and  step_value.ndim < 3:
                    step_value = np.expand_dims(step_value, axis=0)
                if (key=='true_hv' ) and  step_value.ndim < 4:
                # if (key=='true_np' or key=='true_hv') and  step_value.ndim < 3:
                    step_value = np.expand_dims(step_value, axis=0)
