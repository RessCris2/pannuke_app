...
def train(dataset_name, dir_root, save_dir, model_name="dist"):
    ...
    train_loader = DataLoader(
        dataset=DISTDataset(
            train_images_dir, train_masks_dir, classes=classes[dataset_name]
        ),
    ...
    test_loader = DataLoader(
        dataset=DISTDataset(
            test_images_dir, test_masks_dir, classes=classes[dataset_name]
        ),
        batch_size=batch_size,
        shuffle=False,
        drop_last=False,
...
def train_wb(config, dataset_name, dir_root, save_dir, model_name="dist"):
    ...
    for epoch in tqdm(range(config.epochs)):
        ...
        wandb.log({"train_loss": train_loss, "val_loss": validation_loss})
    # 少了一个评估结果, 就是几个指标
